...
public class MovieService {
    ...
    public MovieEntryDto getMovieDetails(String movieName) {
        Movie movieOpt = movieRepository.findByMovieName(movieName);

        return MovieTransformer.convertToDto(movieOpt);
    ...
    public String addMovie(MovieEntryDto movieEntryDto) throws MovieAlreadyPresentWithSameNameAndLanguage {
        if (movieRepository.findByMovieName(movieEntryDto.getMovieName()) != null) {
            if (movieRepository.findByMovieName(movieEntryDto.getMovieName()).getLanguage()
                    .equals(movieEntryDto.getLanguage())) {
