...
public class JwtAuthenticationFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
        ...
        if (userEmail != null && SecurityContextHolder.getContext().getAuthentication() == null) {
            ...
            if (userOptional.isPresent() && jwtTokenUtil.validateToken(jwt, userEmail)) {
                User user = userOptional.get();
                // Create a simplified authentication token with just the necessary user details
                UsernamePasswordAuthenticationToken authToken = new UsernamePasswordAuthenticationToken(
                    user.getEmailId(), // Use email as principal instead of the whole user object
                    null,
                    new ArrayList<>()
                );
